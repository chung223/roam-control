...
- A walk you build yourself. Choose any place — searched, tapped, a favourite, a landmark, a spot — and add it to the walk, in the order you want, dragging to reorder. The multi-stop walk added earlier could only take the nearest three, five or eight of whatever list you were looking at, which is a shortcut rather than a choice.
- Straight-line walks, for the places Apple will not route to. A park's interior, a campus, open country: MapKit answers `directionsNotFound` and the walk could not be planned at all. With Follow streets off, the line goes from each place to the next through whatever is in between, and nothing is asked of Apple Maps.
- GPX import. A track recorded or drawn elsewhere is walked exactly as it was given. Track points are read first, then route points, then waypoints, and a long recording is thinned evenly to a few thousand points — the shape and both endpoints survive, and the map draws it identically.
- A walk that calls at several places. The reason to want five restaurants is that a decoration comes from restaurants, and going to one of them five times is a different errand — which is what the app could do until now. Any spot list offers a walk through its nearest three, five or eight, planned as one leg per stop and laid end to end. The card, the Live Activity and the watch name the stop it is heading for and move on as each is passed.
...
- Session history showed every session in whatever time zone the iPhone is in now. Simulating a distant place moves the device's own zone to match it, so a session run at nine in the morning could be listed as eight the previous evening — the right instant, read off the wrong clock. A record now carries the zone it happened in and is shown in it, naming the zone when it differs from the reader's. Records written before this have no zone to show and keep the old behaviour, which is the only honest thing left for them.
- The diagnostics report says which time zone generated it.
...
- Interpolated interface text is checked at last. A literal containing one is looked up by a key with `%@` in place of it, so neither spelling could find the other, and rather than report every such string as missing the check skipped them all. Both sides are now reduced to the same shape before comparing. It found the walking card's subtitle reading English in every language, the route start and the name a walking location reports for itself, and Siri phrases with no translations at all — those now live in `AppShortcuts.xcstrings`, where the system looks for them.
