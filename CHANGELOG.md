...
- The walking card's subtitle read in English whatever the language. It is built by interpolation, and the localisation check skips any literal containing one, so three strings on the most used screen in the app were never looked at. Sixteen more are unchecked for the same reason; most of those are deliberate.
